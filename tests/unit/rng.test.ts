/**
 * Zufall (A0-Audit: `Math.random` in `src/core/` → 0 Treffer; der morsche Balken und der
 * abfaulende Balken laufen ueber `crypto`).
 */
...
describe('Fairness', () => {
  ...
  it('zieht morschen und abfaulenden Balken produktiv ueber crypto', () => {
    ...
    /* `resolveRound` ohne eigene Quelle nimmt ebenfalls crypto. */
    expect(round).toContain('options.rng ?? SECURE_RNG');
  });
});

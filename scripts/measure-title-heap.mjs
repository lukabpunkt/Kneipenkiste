/**
 * Speicher-Messung des Titel-Loops (Audit A5: „Title-Loop 10 min ohne Leak").
 *
 * **Herkunft:** uebernommen aus dem Schwesterprojekt Drinkshot
 * (`scripts/measure-title-heap.mjs`), angepasst an Basis-Pfad, Disclaimer-Schluessel und
 * Selektor. Der Check stand hier seit M5 als „⏳ manuell" im Audit-Report — jetzt misst
 * ihn eine Maschine, statt dass jemand zehn Minuten auf ein Handy schaut.
 *
...
const url = process.argv[2] ?? 'http://localhost:4173/Tresor/';
...
await page.addInitScript(() => window.localStorage.setItem('tresor.disclaimer.v1', '1'));
await page.goto(url);
await page.waitForSelector('.titleLoop__crook');
 ...
 * Beim Tresor kommt dazu: Der Title-Screen laedt Schriften und das Tuer-SVG nach, und
 * der Crook-Zyklus dauert 14 s. Zwanzig Sekunden decken beides ab.
 */
...
/*
 * Der Loop ist reines CSS auf zwei DOM-Knoten: kein Timer, keine Frame-Schleife, kein
 * PixiJS (der Buehnen-Chunk laedt erst in der Verhandlung, ADR-15). Was hier waechst,
 * waere ein Fehler.
 */
const LIMIT_KB = 512;

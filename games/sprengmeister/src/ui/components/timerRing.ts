/**
 * Timer-Ring (Art Direction §4.2).
 *
 * Zwei Einsatzorte, beide optional (GDD §3.3/§3.4): die Bedenkzeit beim Minenlegen und
 * der Zug-Timer beim Graben. Laeuft er ab, entscheidet nicht die UI, sondern die FSM —
 * `fillPlacements()` bzw. `digRandom()`, beide ueber sicheren Zufall.
 *
 * `hazard`-gelb, in den letzten Sekunden `temp.hot`-rot; kurz vor Schluss pulsiert er und
 * die Ziffer bekommt einen Punch. Stumm muss der Countdown genauso lesbar sein wie mit
 * Ton (GDD §6) — deshalb steckt die ganze Information im Bild.
 */
...
export function createTimerRing(options: TimerRingOptions): TimerRing {

 ...
 *
 * 1. **Sicherer Zufall** (`secureRandomFloat`) — ausschliesslich fuer die Maulwurf-Zuweisung
 *    und den Runden-Seed. Basiert auf `crypto.getRandomValues`. `Math.random` ist in
 *    `src/core/` per ESLint verboten (CLAUDE.md).
 * 2. **Seedbarer PRNG** (`createSeededRng`, mulberry32) — fuer die Show: Reveal-Reihenfolge,
 *    Outcome-Auswahl, Huete, Idle-Gags. Reproduzierbar fuer Debugging und Tests.
 */

const cryptoRef: Crypto = globalThis.crypto;

if (typeof cryptoRef?.getRandomValues !== 'function') {
  throw new Error('crypto.getRandomValues ist nicht verfuegbar — Der Tresor braucht sicheren Zufall.');
}
 ...
 * Nur hierueber laufen Maulwurf-Zuweisung und Seed.
 */
 ...
 * Nicht kryptografisch und darf niemals den Maulwurf bestimmen.
 */

 ...
 *
 * 1. **Sicherer Zufall** (`secureRandomFloat`) — fuer alles, was das Spiel entscheidet:
 *    Hinweise, Diplomat, Item-Set, Runden-Seed (CLAUDE.md). Basiert auf
 ...
 *    Sequenz-Auswahl, Gate-Permutation, Idle-Gags. Reproduzierbar fuer Tests und Dev-Panel.
 */

const cryptoRef: Crypto = globalThis.crypto;

if (typeof cryptoRef?.getRandomValues !== 'function') {
  throw new Error('crypto.getRandomValues ist nicht verfuegbar — Der Zoll braucht sicheren Zufall.');
}
 ...
 * Nur hierueber laufen Hinweise, Diplomat und Item-Set.
 */
 ...
 * Nicht kryptografisch und darf niemals fuer Hinweise, Diplomat oder Item-Set benutzt werden.
 */
...
/* ------------------------------------------------------------------ */
/* Sichere Zufallsquelle mit derselben API                             */
/* ------------------------------------------------------------------ */

/**
 * Was der Regelkern von einer Zufallsquelle braucht — ohne `seed`.
 *
 * `generateHints`, die Diplomaten-Wahl und das Item-Set laufen ueber diese Schnittstelle.
 * Produktiv steckt `SECURE_RNG` drin (crypto), in Tests ein `createSeededRng` — so bleibt
 * die CLAUDE.md-Regel "nur mit crypto.getRandomValues" erfuellt und die Tests deterministisch.
 */
export type RandomSource = Omit<SeededRng, 'seed'>;

/** `RandomSource` auf Basis von `crypto.getRandomValues` — ohne Modulo-Bias. */
export const SECURE_RNG: RandomSource = {
  next: secureRandomFloat,
  int: secureRandomInt,
  range: (min, max) => min + secureRandomFloat() * (max - min),
  intBetween: (min, max) => min + secureRandomInt(max - min + 1),
  pick: <T>(items: readonly T[]): T => {
    if (items.length === 0) throw new RangeError('pick() auf leerem Array');

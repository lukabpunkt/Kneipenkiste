...
## ADR-24 · 2026-09-05 · Kommentare und Commits sind Deutsch (Abweichung von CLAUDE.md)
Kontext: CLAUDE.md verlangt „Docs: Deutsch. Code, Bezeichner, Commits: Englisch." Bezeichner sind durchgehend Englisch, Kommentare und Commit-Nachrichten aber seit M0 Deutsch — sie erklären Spielregeln und Design-Entscheidungen, und die stehen in den Docs auf Deutsch. Entscheidung: Der gewachsene Zustand bleibt; ein Bruch nach sechs Meilensteinen wäre schlechter als eine Sprache, die konsequent durchgehalten ist. Konsequenz: Wer den Code liest, liest dieselbe Sprache wie die Docs, auf die die Kommentare verweisen. **Luka entscheidet, ob das so bleibt** — ein Umstellen ab hier wäre ein sauberer Schnitt, ein rückwirkendes Übersetzen nicht.

## ADR-25 · 2026-09-05 · Der Service Worker wird angemeldet, und ein alter Chunk lädt neu
Kontext: Auf der veröffentlichten Seite stand „Die Halle konnte nicht geladen werden". Ursache: GitHub Pages gibt der `index.html` zehn Minuten Cache; wer nach einem Deploy wiederkommt, bekommt die alte Datei mit Chunk-Namen, die es nicht mehr gibt. Beim Nachsehen fiel zweitens auf, dass `sw.js` zwar ausgeliefert, aber nie angemeldet wurde (`injectRegister: null`, kein `registerSW`) — „offline spielbar" war ein leeres Versprechen. Entscheidung: `registerType: 'autoUpdate'` plus eine Anmeldung in `main.ts`, und ein fehlgeschlagener Chunk-Import lädt die Seite **genau einmal** pro Sitzung neu (Marke in `sessionStorage`). Konsequenz: Offline startet das Spiel wirklich (E2E im Flugmodus), ein Deploy kostet Wiederkehrer höchstens ein Neuladen statt der ganzen Partie, und eine Schleife aus Neuladen ist ausgeschlossen. Der Einstiegs-Chunk wächst um 0,9 KB gzip auf 31,2 KB.

## ADR-26 · 2026-09-05 · Der Hinweis ist ein Zettel am Koffer, nicht ein Icon in der Luft
Kontext: Der Hinweis war ein 20-px-Kreis mit Piktogramm, der frei über dem Gepäckband schwebte — er sah aus wie ein verrutschtes Bedienelement, und bei vier Koffern nebeneinander war nicht zu sehen, welcher gemeint ist. Entscheidung: ein Zoll-Vermerk in Papierfarbe, leicht schief, mit Tuschelinie, Schatten und einer Spitze nach unten auf den Koffer; darauf das Piktogramm und die Kurzform („tickt"), der ganze Satz bleibt als Vorlesetext für Screenreader. Waldis Vermerk trägt die Zollfarbe. Konsequenz: Der Koffer selbst bleibt unangetastet — die A3-Silhouettenprüfung („mit Hinweis sieht aus wie ohne") läuft unverändert durch. Die Zettelbreite ist auf 64 px begrenzt, den Abstand zweier Koffer; deshalb die Kurzform statt „beschnüffelt", das den Nachbarn verdeckt hätte.

## ADR-27 · 2026-09-05 · Die Regeln stehen vollständig im Spiel, mit den Zahlen aus `rules.ts`
Kontext: Der Regel-Screen hatte vier Sätze. Das genügt, um sich zu erinnern, nicht um zu verstehen — und ein Spiel, dessen zweite Design-Säule „falsche Sicherheit" heißt, wird unfair, wenn nur die Hälfte des Tisches begriffen hat, wie unzuverlässig ein Hinweis ist. Entscheidung: acht Abschnitte in der Reihenfolge der Runde (Rollen, Packen, Hinweise, Verhör, Kontrolle, Schranke, Modi, Ende), je ein Absatz plus kurze Punkte; **alle Zahlen werden aus `config/rules.ts` eingesetzt**, nicht in die Übersetzung geschrieben. Konsequenz: Ein Balancing-Pass ändert automatisch die Regeln, die am Tisch vorgelesen werden. Ein E2E-Test prüft, dass kein Platzhalter stehen bleibt — ohne ihn läse jemand irgendwann „{caught} Schlücke" vor. `tList()` kann dafür jetzt ebenfalls interpolieren.

 ...
 *    morscher Balken, Schrumpf-Balken, Bedenkzeit-Fallback, Runden-Seed (CLAUDE.md).
 ...
 *    Sequenz-Auswahl, Bruch-Reihenfolge, Idle-Gags. Reproduzierbar fuer Tests und Dev-Panel.
 */

const cryptoRef: Crypto = globalThis.crypto;

if (typeof cryptoRef?.getRandomValues !== 'function') {
  throw new Error('crypto.getRandomValues ist nicht verfuegbar — Die Haengebruecke braucht sicheren Zufall.');
}
 ...
 * Nur hierueber laufen morscher Balken und Schrumpf-Balken.
 */
 ...
 * Nicht kryptografisch und darf niemals den morschen oder den abfaulenden Balken bestimmen.
 */
 ...
 *
 * `rottenPlank` und `shrink()` laufen ueber diese Schnittstelle. Produktiv steckt
 * `SECURE_RNG` drin (crypto), in Tests ein `createSeededRng` — so bleibt die
 * CLAUDE.md-Regel "nur mit crypto.getRandomValues" erfuellt und die Tests deterministisch.
 */

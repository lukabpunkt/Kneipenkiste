...
/**
 * Der Knopf des Beamten (CLAUDE.md, GDD §5).
 *
 * Hall, Inspect und Gate sind **oeffentliche** Screens — das Handy liegt in der Mitte,
 * und nur einer darf tippen. Deshalb traegt jeder Knopf, der dem Beamten gehoert, seine
 * Farbe und sein Symbol: Man sieht aus einem Meter Entfernung, wessen Knopf das ist.
 */
export interface OfficerButtonOptions extends ButtonOptions {
  colorId: ColorId;
}

export function createOfficerButton(options: OfficerButtonOptions): HTMLButtonElement {
  const color = colorById(options.colorId);
  const button = createButton({ ...options, variant: options.variant ?? 'primary' });

  button.classList.add('btn--officer');
 ...
 * acht Koffer muessen auch dann unterscheidbar bleiben (Audit A2).
 */

/**
 * Kleine DOM-Animationshelfer fuer die Menue-Screens.
 *
 * PIXI und GSAP bleiben auf dem Feld — hier reicht `Element.animate` und ein
 * `requestAnimationFrame`-Zaehler. Das haelt den Renderer aus dem Einstiegs-Chunk
 * (Architektur §1: Board-Chunk lazy waehrend der Lobby).
 *
...
/**
 * Laesst einen Balken auf seine Breite wachsen (Statistik im Result, Roadmap M5.3).
 *
 * Ueber `--score-fill` statt ueber `width`, weil die Farbe im selben
 * Custom-Property-Satz steckt und der Balken so ohne Layout-Rechnung animiert.
 */
 ...
 *    eines Screenwechsels angerufen wird, kaeme sonst zurueck auf einen Screen,
...
export function safeAnimate(
  ...
  try {
    ...
    const budget = Number(options.duration ?? 0) + Number(options.delay ?? 0) + ANIMATION_TIMEOUT_MARGIN_MS;

/**
 * PIXI-Lifecycle (Architektur §8, CLAUDE.md "Performance").
 *
 * **Ein** `Application`-Objekt fuer die gesamte Session. Das Feld wird in drei Screens
 * gebraucht — Place, Dig und Result-Replay — und das Canvas wandert zwischen ihren Hosts,
 * statt neu erzeugt zu werden (ADR-6). Zwischen den Runden wird nur die Buehne geleert.
 *
 * Eine Uhr: der PIXI-Ticker treibt GSAP. Zwei RAF-Schleifen nebeneinander wuerden bei
 * Hit-Stop und Kamera-Shake auseinanderlaufen.
 */
...
// `unsafe-eval` (index.html), deshalb der eval-freie Pfad — ohne ihn stirbt das Feld
// beim ersten Render mit "Current environment does not allow unsafe-eval".
import 'pixi.js/unsafe-eval';
import gsap from 'gsap';
import { RENDER, STAGE } from '@/config/theme';

export interface BoardAssets {
  /** Digger-Rig, Gesichter, Ausruestung, Farb-Symbole. */
  diggers: Spritesheet;
  /** Platten, Tiere, Bomben, Kiste, Buehne, Effekte, Temperatur-Icons. */
  board: Spritesheet;
}
...
let assetsPromise: Promise<BoardAssets> | undefined;
let assetsReady = false;

/**
 * Laedt die Atlanten. Mehrfachaufrufe teilen sich dieselbe Promise, damit der Preload
 * in der Lobby und das spaetere Betreten des Feldes nicht doppelt laden.
 */
export function loadBoardAssets(): Promise<BoardAssets> {
  assetsPromise ??= (async () => {
    const [diggers, board] = await Promise.all([
      Assets.load<Spritesheet>(atlasUrl('diggers')),
      Assets.load<Spritesheet>(atlasUrl('board')),
    ]);
    assetsReady = true;
    return { diggers, board };
  })();
  return assetsPromise;
}

/** Preload im Hintergrund (Roadmap M2.6) — laeuft waehrend der Lobby. */
export function preloadBoardAssets(): void {
  void loadBoardAssets().catch((error) => {
    console.warn('[board] Preload fehlgeschlagen', error);
    // Beim echten Betreten des Feldes wird erneut versucht.
    assetsPromise = undefined;
  });
}

/** True, sobald beide Atlanten im Speicher liegen. */
export function areBoardAssetsReady(): boolean {
  return assetsReady;
}

export interface BoardAppHandle {
  readonly app: Application;
  /** Alles Spielbare hängt hier drin; wird in Weltkoordinaten (1000×1000) gerechnet. */
  readonly world: Container;
  /** Bildschirmraum ueber der Welt — hier liegen Banner-Overlays (unskaliert, CSS-Pixel). */
  ...
  /** Wird nach jedem Resize gerufen — die Kamera haengt sich hier ein. */
  onLayout(listener: (layout: BoardAppHandle['layout']) => void): () => void;
  ...
  drawCalls(): number;
  /**
   * Reine JS-Zeit pro Frame (Simulation, ohne Rendern). `RENDER.budgetMs.update` gibt
   * dafuer 4 ms vor — und anders als die Frame-Zeit ist dieser Wert unabhaengig davon,
   * ob der Testrechner eine GPU hat.
   */
  updateTimes(): readonly number[];
  /** Wird vom Dig-Screen mit der gemessenen Update-Dauer gefuettert. */
  recordUpdate(durationMs: number): void;
  destroy(): void;
}

let handle: BoardAppHandle | undefined;

/**
 * Erzeugt die App beim ersten Aufruf und gibt danach immer dieselbe Instanz zurueck.
 * Ein zweites `PIXI.Application` waere ein Audit-A2-Fehler.
 */
export async function getBoardApp(): Promise<BoardAppHandle> {
  ...
  const relayout = (): void => {
    ...
    /*
     * Die Welt ist im Hochformat (ADR-12). "Fit" statt "cover": Lieber ein Rand als ein
     * abgeschnittenes Feld — eine Platte, die halb aus dem Bild ragt, ist nicht tippbar.
     */
    const scale = Math.min(width / STAGE.worldSize, height / STAGE.worldHeight);
    const x = (width - STAGE.worldSize * scale) / 2;
    const y = (height - STAGE.worldHeight * scale) / 2;
  ...
  handle = {
    ...
    /*
     * Die Kamera verschiebt und skaliert `world` zur Laufzeit (Zoom auf die Platte,
     * Shake). Deshalb wird hier die **tatsaechliche** Transformation gelesen und nicht
     * die Ruhelage — sonst laege ein Banner beim Zoom neben dem Krater.
     */
...
/** JS-Zeit pro Frame des laufenden Feldes — Testhilfe fuer `perf.spec.ts`. */
export function boardUpdateTimes(): readonly number[] {
  return handle?.updateTimes() ?? [];
}

/** Aktuelle Welt-Transformation — Testwerkzeuge leiten daraus Bildausschnitte ab. */
export function boardLayout(): BoardAppHandle['layout'] | undefined {
  return handle?.layout;
}

/** Wieviele PIXI-Applications leben gerade? Muss immer 0 oder 1 sein (Audit A2). */
export function boardAppExists(): boolean {
  return handle !== undefined;
}

/** Nur fuer Tests: gibt den Singleton frei. */
export function resetBoardApp(): void {
  handle?.destroy();
  handle = undefined;
  assetsPromise = undefined;
  assetsReady = false;
...
/**
 * Legt einen Zaehler um die WebGL-Draw-Aufrufe. Das ist die Zahl, die im A2-Audit zaehlt —
 * nicht eine interne Batch-Liste, die auch mal veraltet sein kann. Schlaegt still fehl,
 * wenn der Renderer kein WebGL nutzt; dann liefert `drawCalls()` eben 0.
 */
...
/**
 * Misst nach dem Betreten des Feldes und meldet, ob der Low-Effects-Modus greifen soll.
 * Wird als `true` aufgeloest, wenn der Frame-Median ueber der Schwelle liegt.
 */
export function measureLowEffects(target: BoardAppHandle): Promise<boolean> {
